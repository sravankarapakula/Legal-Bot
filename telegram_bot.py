...
async def docs_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text(
        ...
        "Type /help or ? if you need help with a field._\n\n"
...
async def help_during_collect(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle /help command during form filling. Ask for the user's question."""
    questions = context.user_data["questions"]
    q_index = context.user_data["q_index"]
    _, field_label = questions[q_index]

    logger.info("[HELP] /help command received from user %s", update.effective_user.id)
    print(f"\n  [HELP] User {update.effective_user.first_name} requested help on field: {field_label}")

    await update.message.reply_text(
        f"You are currently on: {field_label}\n\n"
        f"What do you need help with? Type your question below:\n\n"
        f"(Or type 'back' to go back to the question)"
    )
    return WAITING_HELP_QUERY


async def help_query_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """User typed their help question. Send it to the LLM and respond."""
    user_query = update.message.text.strip()

    # If user wants to go back without asking
    if user_query.lower() == "back":
        ...
        await update.message.reply_text(
            ...
            f"{field_label}\n\n"
            f"Type /help if you need help."
        )
        return COLLECT_INFO

    # Call the LLM with the user's question
    return await _handle_help_request(update, context, user_query=user_query)
...
async def collect_info_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # ── Detect help requests via text ──
    answer_lower = answer.lower()
    if answer_lower in ("?", "help"):
        # Route to help flow — ask for their question
        return await help_during_collect(update, context)

    # Save answer (unless skipped)
    if answer_lower != "skip":
    ...
    if q_index < len(questions):
        ...
        await update.message.reply_text(
            f"Question {q_index + 1}/{len(questions)}:\n"
            f"{label}\n\n"
            f"Type /help or ? if you need help"
        )
        return COLLECT_INFO
...
async def _handle_help_request(
    ...
    """Query the LLM for help on the current field, then stay on the same question."""
    questions = context.user_data["questions"]
    q_index = context.user_data["q_index"]
    ...
    # Terminal logging
    print(f"\n{'='*60}")
    print(f"  [LLM HELP REQUEST]")
    print(f"  User: {update.effective_user.first_name} (ID: {update.effective_user.id})")
    print(f"  Field: {field_label} ({field_key})")
    print(f"  Category: {context.user_data.get('category', 'N/A')}")
    print(f"  User Question: {user_query}")
    print(f"{'='*60}")

    await update.message.reply_text("Thinking...")

    # Gather all previous answers for context
    previous_answers = {}
    for key, _ in questions[:q_index]:
        if key in context.user_data["case_data"]:
            previous_answers[key] = context.user_data["case_data"][key]

    # Call the LLM
    try:
        help_text = get_field_help(
            dispute_category=context.user_data["category"],
            workflow_title=context.user_data["workflow"]["title"],
    ...
    # Terminal logging of response
    print(f"\n  [LLM RESPONSE]")
    print(f"  {help_text[:500]}")
    print(f"{'='*60}\n")

    # Send response as plain text
    await update.message.reply_text(
        f"Help for: {field_label}\n"
        f"{'_'*30}\n\n"
        f"{help_text}"
    )
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            COLLECT_INFO: [
                CommandHandler("help", help_during_collect),
                MessageHandler(filters.TEXT & ~filters.COMMAND, collect_info_received),
            ],
            WAITING_HELP_QUERY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, help_query_received),
            ],
            ASK_TRACKER: [
